...
def get_genes_from(fn, sample_n=0):
	df = pd.read_csv(fn)
	genes = [ga.Gene(row['city'], row['latitude'], row['longitude']) for _, row in df.iterrows()]
...
def plot_route(individual):
	m = Basemap(projection='lcc', resolution=None, width=5E6, height=5E6, lat_0=-1, lon_0=-56)
	
	plt.axis('off')
	plt.title("Shortest Route")
	
	for i in range(0, len(individual.genees)):
