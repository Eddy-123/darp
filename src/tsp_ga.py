...
class Gene:
	#keep distances from cities saved in a table
	__distances_table = {}
	...
	def travel_cost(self):
		if self.__travel_cost == 0:
			for i in range(len(self.genes)):
				...
				self.__traveling_cost += origin.get_distance_to(dest)
...
def evolve(pop, tourn_size, mut_rate):
	...
	for i in range(elitism_num, pop_size):
		mutate(new_generation.individuals[1], mut_rate)
...
def crossover(parent_1, parent_2):
	...
	child = Individual([None for _ in range(gene_n)])
