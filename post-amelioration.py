...
def tabu_search(individu, population_size):
	neighbours = []
	tabu_list = []
	while len(tabu_list) < (population_size):
		while len(neighbours) < 50:
			x, y = random.sample(range(len(individu)), 2)
			neighbour = individu[:]
			neighbour[x], neighbour[y] = neighbour[y], neighbour[x]
			neighbours.append(neighbour)
		while (get_best_current(neighbours)[1] in tabu_list):
			neighbours.remove(get_best_current(neighbours)[1])
		tabu_list.append(get_best_current(neighbours)[1])
	return tabu_list
...
def main(iter_count):
    ...
    while i < iter_count - 1:
        ...
        i += 1
    population_from_tabu_search = tabu_search(result, population_size)
    
    #Update breeding results
    mile_cost,result,population = get_best_current(population_from_tabu_search) 
    record.append(mile_cost) #Record the optimal solution after each reproduction
    complete_route = [origin] + result + [origin]
    return complete_route,mile_cost,record
